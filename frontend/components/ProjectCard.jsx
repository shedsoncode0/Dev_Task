...
function ProjectCard({ brandName, members, pay, startD, deadL, project, important }) {
    const { 
        setPopup, 
        user,
        projects,
  ...
  return (
    <div className='bg-white shadow-lg hover:shadow-xl transition-all rounded-md max-w-xs w-full cursor-pointer'>
        <div className='flex text-white justify-between relative'>
            <h1 className='bg-cyan-500 font-medium text-sm px-2 rounded-tr-md rounded-tl-md'>Full-Stack</h1>
            <HiDotsHorizontal onClick={() => openOptions()} color='black' size={23} className="mr-3 top-1 relative"/>
        </div>
        <div className='relative'>
            {option && (
                <div className='w-[170px] bg-white shadow-lg absolute z-30 -right-10 rounded-md overflow-hidden'>
                    <ul className=''>
                        <li onClick={() => deleteProject(project._id)} className='flex px-3 py-2 hover:bg-slate-100 items-center space-x-4 w-full'><HiOutlineTrash size={20}/> <span>Delete</span></li>
                        <li className='flex px-3 py-2  hover:bg-slate-100 items-center space-x-4 w-full'><RiEdit2Line size={20} /> <span>Edit</span></li>
                    </ul>
                </div>
            )}
            ...
            <div className='flex w-full py-4 justify-evenly space-x-4 relative'>
                ...
                {important && (
                    <RiAlarmWarningLine className='absolute bottom-4 left-0 text-red-400' size={20}/>
                )}
